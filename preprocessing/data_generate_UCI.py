...
def preprocess(data_name):
    ...
    if data_name == 'BC':
        rawdata = pd.read_csv(os.path.join(raw_dir, 'Breast Cancer', 'breast-cancer-wisconsin.csv'), header=None)
        ...
        rawdata = pd.read_csv(os.path.join(raw_dir, 'Horse Colic', 'horse-colic.csv'), header=None)
        data1 = rawdata.replace('?', np.nan, inplace=False)
        ...
        rawdata = pd.read_csv(os.path.join(raw_dir, 'Hepatitis', 'hepatitis.csv'), header=None)
        data1 = rawdata.replace('?', np.nan, inplace=False)
